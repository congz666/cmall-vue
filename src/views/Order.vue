        ...
        <div class="order-bar">
          <div class="order-bar-left">
            <span class="order-total">
              共
              <span class="order-total-num">{{item.num}}</span> 件商品
            </span>
          </div>
          <div class="order-bar-right">
            <span>
              <span class="total-price-title">合计：</span>
              <span class="total-price">{{item.discount_price*item.num}}元</span>
            </span>
          </div>
          <!-- 订单列表END -->
        </div>
...
export default {
  ...
  methods: {
    getOrders(){
      // 获取订单数据
      ordersAPI.showOrders(this.$store.getters.getUser.id).then(res =>{
